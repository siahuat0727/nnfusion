...
class NNFusionRT:
        ...
        def check_if_need_build():
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                if not filecmp.cmp(temp_onnx_path, self.onnx_path):
                    ...
                    shutil.copy(temp_onnx_path, self.onnx_path)
                    return True
            return False
    ...
    def run(self, inputs, outputs):
        ...
        # TODO what will cause executor.get_inputs return weights
        in_dict = {
    ...
    def _signature(self):
        """
        Signature of a function or torch.nn.Module instance to avoid collision
        (misreplaced by each other).
        """
        def get_qualname():
            ...
            # Replace special chars to avoid the trouble of dealing paths
            return re.sub("[<>]", "", name)

        def get_path():
            # Avoid collision among different files
            if isinstance(self.model, TorchModule):
